...
import type { ChainReader, LpPosition, ProtocolAdapter } from "../core/types";
...
describe("getWalletPositions (agregação)", () => {
  it("agrega posições de vários protocolos", async () => {
    const dto = await getWalletPositions(fakeReader, ADDR, [okAdapter("aerodrome"), okAdapter("uniswap-v3")]);
  ...
  it("um protocolo caído → resposta parcial + warning (não derruba tudo)", async () => {
    const dto = await getWalletPositions(fakeReader, ADDR, [okAdapter("aerodrome"), brokenAdapter("uniswap-v3")]);
  ...
  it("todos os protocolos caídos → erro (vira 502 na API)", async () => {
    await expect(
      getWalletPositions(fakeReader, ADDR, [brokenAdapter("aerodrome"), brokenAdapter("uniswap-v3")]),
